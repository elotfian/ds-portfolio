...
| [Leak-Safe Classification Pipeline](leak_safe_classification_pipeline.ipynb) | Built a full scikit-learn Pipeline + ColumnTransformer for a classification task, with every preprocessing step fit only on training data. Evaluated with stratified cross-validation instead of a single split, then tuned via a regularization sweep chosen by F1 score rather than accuracy alone. | scikit-learn Pipeline/ColumnTransformer, cross-validation, regularization (L2), precision/recall/F1 tradeoff analysis |
